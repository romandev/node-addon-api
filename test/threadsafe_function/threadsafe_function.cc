...
static void SecondaryThread(void* data) {
  auto* tsFunction = static_cast<ThreadSafeFunction*>(data);

  if (tsFunction->Release() != napi_ok) {
...
static void DataSourceThread(void* data) {
  ...
  if (!queueWasClosing && tsFunction->Release() != napi_ok) {
